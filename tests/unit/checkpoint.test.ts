...
console.log('\nevery claim in a record must be in the journal')
{
  ...
  const forged = { ...c, read: [...c.read, { path: 'src/main/index.ts', range: null }], changed: [{ path: 'src/shared/host.ts', how: 'created' as const, times: 1 }], commands: [{ command: 'npm test', exitCode: 0, timedOut: false }] }
  assert.deepEqual(verifyCheckpoint(forged, events), ['read src/main/index.ts', 'changed src/shared/host.ts', 'command npm test → 0'])
  ok('a read, a change and a command the journal does not hold are each named as unsupported')
